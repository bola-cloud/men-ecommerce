...
**(تم التراجع عن هذا التعديل: تم إعادة إظهار كافة الأقسام بناءً على الطلب، مع الإبقاء على إخفاء "مندوبي التوصيل" فقط من قسم المستخدمين).**
...
**تم إخفاء الأقسام التالية عبر التعليق البرمجي `{{-- --}}` (قبل التراجع):**
...
**الملفات التي تم التعديل عليها وإرجاعها لحالتها الأصلية:**
1. `resources/views/layouts/admin/partials/_side-bar.blade.php`: إرجاع إعدادات النظام والطرف الثالث والقوالب (V1).
2. `resources/views/layouts/admin/partials/v2/_side-bar.blade.php`: إرجاع جميع الأقسام المذكورة أعلاه من القائمة الجانبية الجديدة (V2)، ما عدا الكود الخاص بـ `deliveryman` داخل قسم الـ People الذي تم إبقاؤه معلقاً.
3. `resources/views/layouts/admin/partials/v2/_rail.blade.php`: إرجاع الأيقونات المصغرة للأقسام الأربعة المذكورة (الكتالوج، التسويق، الأشخاص، التقارير) من القائمة الجانبية الجانبية (Slim sidebar) لتصميم V2.

**ملاحظة:** الأقسام تم إعادتها بنجاح للظهور في واجهة الإدارة.
